...
Set<Shift> helper(const Set<Shift>& shifts, int maxHours)
{
    if (shifts.isEmpty() || maxHours <= 0) {
        return {};
    }
    Shift curr = shifts.first();
    Set<Shift> rest = shifts - curr;
    Set<Shift> without = helper(rest, maxHours);
    Set<Shift> with;
    int val = -1;
    if (maxHours - lengthOf(curr) >= 0) {
        Set<Shift> n;
        for (Shift el : rest) {
            if (!overlapsWith(el, curr)) {
                n.add(el);
            }
        }
        with = helper(n, maxHours - lengthOf(curr));
        with.add(curr);
        val = tot(with);
    }
    if (val > tot(without)) {
