...
using namespace std;
double calc(const Map<string, int>& populations, const Map<string, int>& seats, string city){
    return populations[city]/sqrt((seats[city]* (seats[city]+1)));
}
Map<string, int> apportion(const Map<string, int>& populations, int numSeats) {
    ...
    HeapPQueue q = HeapPQueue();
    Map<string, int>  seats;
    for(string s : populations.keys()){
        seats.put(s, 1);
        numSeats--;
        DataPoint p;
        p.name=s;
        p.weight=(populations[s]*-1)/sqrt(2);
        q.enqueue(p);
    }
    while(numSeats!=0){
        DataPoint city = q.dequeue();
        seats[city.name]+=1;
        numSeats--;
        double val = calc(populations, seats, city.name);
        city.weight=val*-1;
        q.enqueue(city);
    }
    return seats;
