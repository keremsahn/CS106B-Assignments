...
void HeapPQueue::enqueue(const DataPoint& data) {
    ...
    if(logicalSize+1==allocatedSize){
        DataPoint* ptr = new DataPoint[allocatedSize*2]();
    ...
    int tmp = logicalSize+1;
    while(tmp!=1){
        if(elems[tmp].weight < elems[tmp/2].weight){
            DataPoint tm = elems[tmp];
            elems[tmp] = elems[tmp/2];
            elems[tmp/2] = tm;
            tmp/=2;
        }
        else{
...
DataPoint HeapPQueue::dequeue() {
    ...
    while(start <= logicalSize){
        ...
        if(ind1 > logicalSize){
            ...
            if(ind2 > logicalSize){
                if(elems[start].weight > elems[ind1].weight){
                    DataPoint tmp = elems[start];
                    elems[start] =elems[ind1];
                    elems[ind1] = tmp;
                    start=ind1;
                }
                else{
                ...
                if(elems[ind1].weight < elems[ind2].weight){
                        ind = ind1;
                }
                else{
                    ind=ind2;
                }
                if(elems[start].weight > elems[ind].weight){
                    DataPoint tmp = elems[start];
                    elems[start] =elems[ind];
                    elems[ind] = tmp;
                    start=ind;
                }
                else{
