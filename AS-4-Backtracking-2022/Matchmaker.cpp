...
bool rec(const Map<string, Set<string>>& possibleLinks, Set<Pair>& matching, Set<string>& people) {
    if(people.isEmpty()){
        return true;
    }
    else{
        string el = people.first();
        people.remove(el);
        for(string el2 : possibleLinks[el]){
            if(people.contains(el2)){
                people.remove(el2);
                Pair tmp(el, el2);
                matching.add(tmp);
                if(rec(possibleLinks, matching, people)){
                    return true;
                }
                people.add(el2);
                matching.remove(tmp);
...
bool hasPerfectMatching(const Map<string, Set<string>>& possibleLinks, Set<Pair>& matching) {
    ...
    Set<string> people;
    for (string el : possibleLinks.keys()){
        people.add(el);
    }
    return rec(possibleLinks, matching, people);
}
int calc(const Map<string, Map<string, int>>& possibleLinks, Set<Pair>& p){
    int tot=0;
    for(Pair el : p){
        tot+=possibleLinks[el.first()][el.second()];
    }
    return tot;
}
Set<Pair> weight(const Map<string, Map<string, int>>& possibleLinks, Set<string>& people){
    if(people.isEmpty()){
        Set<Pair> emp;
        return emp;
    }
        ...
        string el = people.first();
        people.remove(el);
        Set<Pair> poss1 = weight(possibleLinks, people);
        int val1 = calc(possibleLinks, poss1);
        int m = 0;
        for(string el2 : possibleLinks[el].keys()){
            if(people.contains(el2) && possibleLinks[el][el2]>0){
                people.remove(el2);
                Set<Pair>poss2 = weight(possibleLinks, people);
                Pair n(el, el2);
                poss2.add(n);
                people.add(el2);
                int val2 = calc(possibleLinks, poss2);
                if(val2 > m){
                    m=val2;
                    final=poss2;
                }
...
Set<Pair> maximumWeightMatching(const Map<string, Map<string, int>>& possibleLinks) {
    ...
    return weight(possibleLinks, people);
