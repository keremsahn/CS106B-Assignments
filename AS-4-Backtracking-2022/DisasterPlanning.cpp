 ...
 */
bool rec (const Map<string, Set<string>>& roadNetwork,
                            int numCities,
                            Set<string>& supplyLocations, Set<string> covered){
    if(numCities < 0){
        return false;
    }
    else if(numCities>=0 && covered.isEmpty()){
        return true;
    }
    else{
        string city = covered.first();
        supplyLocations.add(city);
        covered.remove(city);
        Set<string> tmp;
        for(string cit : roadNetwork[city]){
            if(covered.contains(cit)){
                tmp.add(cit);
                covered.remove(cit);
            }
        }
        if(rec(roadNetwork, numCities-1, supplyLocations, covered)){
            return true;
        }
        covered.add(city);
        covered+=tmp;
        supplyLocations.remove(city);
        for(string c : roadNetwork[city]){
            Set<string> buf;
            bool b1 = false;
            ...
            supplyLocations.add(c);
            for(string el : roadNetwork[c]){
...
bool canBeMadeDisasterReady(const Map<string, Set<string>>& roadNetwork,
    ...
    Set<string> emp;
    for(string s : roadNetwork.keys()){
        emp.add(s);
    }
    return rec(roadNetwork, numCities, supplyLocations, emp);
}
