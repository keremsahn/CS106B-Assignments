...
Vector<DataPoint> merg(const Vector<DataPoint>& v1, const Vector<DataPoint>& v2){
    Vector<DataPoint> emp;
    int p1 = 0;
    int p2 = 0;
    while(p1< v1.size() && p2 < v2.size()){
        if(v1[p1].weight < v2[p2].weight){
            emp.add(v1[p1]);
            p1++;
        }
        else{
...
Vector<DataPoint> combine(const Vector<Vector<DataPoint>>& sequences) {
    ...
    if(sequences.size()==0){
        ...
        return merg(sequences[0], sequences[1]);
    }
    else{
