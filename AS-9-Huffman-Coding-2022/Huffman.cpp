...
EncodingTreeNode* huffmanTreeFor(const string& str) {
    ...
    if(map.size()<2){
        error("Not enough different char");
    }
    PriorityQueue<EncodingTreeNode*> queue;
    for(char el : map.keys()){
        EncodingTreeNode* ptr= new EncodingTreeNode;
        ptr->ch = el;
        ptr->one=NULL;
        ptr->zero=NULL;
        queue.enqueue(ptr, map[el]);
    }
    while(queue.size()>1){
        int val1= queue.peekPriority();
        EncodingTreeNode* node1 = queue.dequeue();
        int val2 = queue.peekPriority();
        EncodingTreeNode* node2 = queue.dequeue();
        int tot_val=val1+val2;
        EncodingTreeNode* new_node = new EncodingTreeNode;
        new_node->one=node2;
        new_node->zero=node1;
        queue.enqueue(new_node, tot_val);
    }
    EncodingTreeNode* last = queue.dequeue();
    return last;
}
 ...
 */
string helper(Queue<Bit>& bits, EncodingTreeNode* tree,EncodingTreeNode* root){
    if(tree->one==NULL && tree->zero==NULL){
        EncodingTreeNode* n = root;
        return string(1, tree->ch) + helper(bits, n, root);
    }
        ...
        Bit b = bits.dequeue();
        if(b==0){
            return helper(bits, tree->zero, root);
        }
        return helper(bits, tree->one, root);
    }
}
string decodeText(Queue<Bit>& bits, EncodingTreeNode* tree) {
    /* TODO: Delete this comment and the next few lines, then implement this. */
    return helper(bits, tree, tree);
}
...
Vector<Bit> vec;
void help(EncodingTreeNode* tree){
    if(tree->one==NULL && tree->zero==NULL){
        m.put(tree->ch, vec);
    }
    else{
...
EncodingTreeNode* decodeTree(Queue<Bit>& bits, Queue<char>& leaves) {
    ...
    if(bits.size()==0 && leaves.size()){
        ...
        if(b==1){
            EncodingTreeNode* ptr = new EncodingTreeNode;
            ptr->zero=decodeTree(bits, leaves);
            ptr->one=decodeTree(bits, leaves);
            return ptr;
        }
        else{
            EncodingTreeNode* n = new EncodingTreeNode;
            n->ch=leaves.dequeue();
            n->one=NULL;
            n->zero=NULL;
            return n;
...
HuffmanResult compress(const string& text) {
    ...
    Queue<Bit> treeBits;
    Queue<char> leaves;
    EncodingTreeNode* ptr = huffmanTreeFor(text);
    encodeTree(ptr, treeBits, leaves);
    Queue<Bit> messageBits = encodeText(text, ptr);
    n.messageBits=messageBits;
    n.treeBits=treeBits;
    n.treeLeaves=leaves;
    deleteTree(ptr);
    return n;
...
string decompress(HuffmanResult& file) {
    ...
    EncodingTreeNode* ptr = decodeTree(treeBits, leaves);
    string s = decodeText(messageBits, ptr);
    deleteTree(ptr);
    return s;
}
