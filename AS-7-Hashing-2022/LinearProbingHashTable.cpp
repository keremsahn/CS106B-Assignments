...
bool LinearProbingHashTable::insert(const string& elem) {
    ...
    int tmp=hash_val;
    while(true){
        if(elems[tmp%capacity].type==SlotType::EMPTY || elems[tmp%capacity].type==SlotType::TOMBSTONE){
            elems[tmp%capacity].value=elem;
            elems[tmp%capacity].type=SlotType::FILLED;
            curr_size++;
            return true;
        }
...
bool LinearProbingHashTable::contains(const string& elem) const {
    ...
    int tmp = hash_val;
    while(true){
        if(elems[tmp%capacity].type==SlotType::FILLED && elems[tmp%capacity].value==elem){
            return true;
        }
        if(elems[tmp%capacity].type==SlotType::EMPTY){
            return false;
        }
        if(tmp != hash_val && tmp%capacity==hash_val){
            return false;
...
bool LinearProbingHashTable::remove(const string& elem) {
    ...
    while(true){
        if(elems[tmp%capacity].type==SlotType::FILLED && elems[tmp%capacity].value==elem){
            elems[tmp%capacity].type=SlotType::TOMBSTONE;
            curr_size--;
            return true;
        }
        if(tmp!=hash_val && tmp%capacity==hash_val){
