...
bool RobinHoodHashTable::insert(const string& elem) {
    ...
    string will_put=elem;
    int hash_val=func(elem);
    int tmp=hash_val;
    int dis=0;
    while(true){
        if(elems[tmp%capacity].distance==EMPTY_SLOT){
            elems[tmp%capacity].value=will_put;
            elems[tmp%capacity].distance=dis;
            curr_size++;
            return true;
        }
        if(elems[tmp%capacity].distance < dis){
            swap(elems[tmp%capacity].distance, dis);
            swap(elems[tmp%capacity].value, will_put);
            hash_val=func(will_put);
            tmp=hash_val;
            dis = 0;
            continue;
        }
        else if(tmp!=hash_val && tmp%capacity==hash_val){
            return false;
        }
        tmp++;
        dis++;
...
bool RobinHoodHashTable::contains(const string& elem) const {
    ...
    while(true){
        ...
        if(elems[tmp%capacity].distance!=EMPTY_SLOT && elems[tmp%capacity].value==elem){
            return true;
        }
        if(elems[tmp%capacity].distance!=EMPTY_SLOT && elems[tmp%capacity].distance < dis){
            return false;
        }
        if(tmp!=hash_val && tmp%capacity==hash_val){
            return false;
        }
        tmp++;
        dis++;
...
bool RobinHoodHashTable::remove(const string& elem) {
    ...
    int tmp=hash_val;
    while(true){
        if(elems[tmp%capacity].value==elem){
            elems[tmp%capacity].distance=EMPTY_SLOT;
            int tmp2=tmp;
            tmp2++;
            while(elems[tmp2%capacity].distance!=EMPTY_SLOT && elems[tmp2%capacity].distance>0){
                elems[(tmp2%capacity -1 + capacity)%capacity].distance=elems[tmp2%capacity].distance-1;
                elems[(tmp2%capacity -1 + capacity)%capacity].value=elems[tmp2%capacity].value;
                elems[tmp2%capacity].distance=EMPTY_SLOT;
                tmp2++;
            }
            curr_size--;
            return true;
        }
        if(tmp!=hash_val && tmp==hash_val%capacity){
            return false;
        }
        tmp++;
    }
