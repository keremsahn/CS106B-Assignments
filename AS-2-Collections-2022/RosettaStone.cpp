...
Map<string, double> kGramsIn(const string& str, int kGramLength) {
    ...
    for(int i = 0; i + kGramLength< str.length() +1; i++){
...
string guessLanguageOf(const Map<string, double>& textProfile,
    ...
    for(Corpus s: corpora){
        if(cosineSimilarityOf(textProfile, s.profile) > max){
            max=cosineSimilarityOf(textProfile, s.profile);
            str=s.name;
        }
    }
    return str;
