...
Grid<bool> floodedRegionsIn(const Grid<double>& terrain,
                            ...
                            double height) {
    Grid<bool> fin(terrain.numRows(), terrain.numCols(), false);
    ...
    while(!q.isEmpty()){
        GridLocation loc = q.dequeue();
        for(int i=-1; i < 2; i+=2){
            GridLocation n1=loc;
            GridLocation n2=loc;
            n1.row+=i;
            n2.col+=i;
            if(terrain.inBounds(n1) && terrain.get(n1) <= height && !fin.get(n1)){
                fin[n1]=true;
                q.enqueue(n1);
            }
            if(terrain.inBounds(n2) && terrain.get(n2) <= height && !fin.get(n2)){
                fin[n2]=true;
                q.enqueue(n2);
            }
        }
